...
def train():
    ...
    for data in tqdm(train_loader):
        ...
        input = torch.stack(
            (
                [
                    train_transform(transforms.ToPILImage()(x_.permute(2, 0, 1)))
                    for x_ in data.image
                ]
            )
        ).to(device)
        out = model(input)
    ...
    for data in test_loader:
        with torch.no_grad():
            input = torch.stack(
                (
                    [
                        val_transform(transforms.ToPILImage()(x_.permute(2, 0, 1)))
                        for x_ in data.image
                    ]
                )
            ).to(device)
...
def extract_features(loader, loader_name):
    for _, data in enumerate(loader):
        ...
        for i in range(0, features.shape[0]):
            ...
            points_average = torch.nn.AvgPool2d(
                7, count_include_pad=False, ceil_mode=False
            )
            points_torch_32 = points_average(
                torch.tensor(data.img_point_cloud[i]).permute(2, 0, 1)
            )
            points_d32 = np.reshape(points_torch_32.data.numpy(), (-1, 3))

            save_name = (
                f"{get_data_dir()}/fusion/2d/{loader_name}/{data.dataset_idx[i]}.h5"
            )
            save_h5_features(save_name, "features", feature.detach().cpu().numpy())
            save_h5_features(save_name, "points", points_d32)
            save_h5_features(
                save_name, "label", np.atleast_1d(data.y[i].detach().cpu().numpy())
            )
...
if __name__ == "__main__":
    ...
    model = timm.create_model(
        "convnext_large", pretrained=True, num_classes=19, drop_path_rate=0.8
    ).cuda()
    ...
    model, optimizer, start_epoch = load_ckp(
        f"data/checkpoints/{model_name}_best_model.pt", model, optimizer, scheduler
    )
